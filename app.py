...
async def main():
    ...
    st.text("SEARCH ANYTHING IN THE USER MANUAL")
    ...
    if st.session_state.step == 1:
        ...
        selected_stream = st.selectbox("Car Type", streams)
        if st.button("Next", key="nextsem", on_click=next):
            pass

    elif st.session_state.step == 2:
        st.subheader(f"Select Your car Type")
        selected_year = st.selectbox(
            "Choose a car", ["BALENO", "BREZZA", "DZIRE", "SWIFT"]
        )
        ...
        st.subheader(f"Select Your Fuel type")
        s1 = [
            "CNG",
            "PETROL"
        ]
        ...
        if (
        ...
        ) and st.session_state.selected_year == "BALENO":
            ...
            or st.session_state.selected_stream
            == "Volvo"
        ) and st.session_state.selected_year == "BREZZA":
